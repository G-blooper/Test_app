...
def main_page():
    ...
    level_opt = st.radio("解説レベル", ["入門", "初級", "中級", "上級"], horizontal=True)

    # --- プロンプト生成 ---
    def build_prompt(tcase, err, level):
        common = "次のプログラムについて、テストケースが成功するようにエラーの修正方法を"
        audience = {"入門":"プログラミングを学び始めたばかりの人に説明してください。専門用語を使わずに、どこをどのように修正すればいいのかを教えてください。手順を簡潔に書いてください。",
                    "初級": "プログラミングの基礎構文を理解している人に説明してください。エラーが起きた原因とその修正理由を教えてください。", 
    ...
    st.code(selected_prompt, wrap_lines=True)
    ...
    if st.button("AIに送信"):
        if not program:
            ...
            log = f"実行: {st.session_state.user_id} がAI診断を実行"
            append_line_to_repo_log(REPO_OWNER, REPO_NAME, LOG_DIR+"/IDlogin.txt", log)
